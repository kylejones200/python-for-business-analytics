...
def main():
    ...
    summary_lines = [
        ...
        "",
        "Key Metrics:",
        f"  - MRR Range: ${df['mrr_usd'].min():,.0f} to "
        f"${df['mrr_usd'].max():,.0f}",
    ...
    for seg in df['segment'].value_counts().sort_index().items():
        summary_lines.append(
            f"  - {seg[0]}: {seg[1]:,} customers ({seg[1]/len(df)*100:.1f}%)"
        )
    ...
    summary_lines.extend(
        [
            ...
            f"  - Active: {(~df['churned']).sum():,} "
            f"({(~df['churned']).sum()/len(df)*100:.1f}%)",
            f"  - Onboarding Complete: {df['onboarding_complete'].sum():,} "
            f"({df['onboarding_complete'].sum()/len(df)*100:.1f}%)",
        ]
    )
