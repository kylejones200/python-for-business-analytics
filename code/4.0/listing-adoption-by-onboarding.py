...
def main():
    ...
    ax.hist(onboarded, bins=30, alpha=0.6, color="#88C0D0", 
           label="Onboarding Complete", edgecolor="black")
    ax.hist(not_onboarded, bins=30, alpha=0.6, color="#D08770",
           label="Onboarding Incomplete", edgecolor="black")
    
    ax.set_xlabel("Adoption Gap", fontsize=12)
    ax.set_ylabel("Frequency", fontsize=12)
    ax.set_title("Adoption Gap by Onboarding Status", fontsize=14, fontweight="bold")
    ax.legend()
    ...
    logger.info(f"Not Onboarded: mean={not_onboarded.mean():.4f}, n={len(not_onboarded)}")
