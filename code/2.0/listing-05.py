...
This script shows how to load the business_ops dataset using the
ensure_dataset utility. Readers learn dataset loading, parquet file handling,
and data exploration with head().
...
def main():
    ...
    try:
        ...
        logger.error(
            "bookdata module not found. Ensure src/bookdata.py exists."
        )
        raise SystemExit(1)
    ...
    if ops_path is None:
        logger.warning(
            "SKIPPED: business_ops dataset unavailable. Run `python "
