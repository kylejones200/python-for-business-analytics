...
def series_for(df, unit_id, controls, weights):
    ...
    synth = pd.Series(
        control_mat.to_numpy() @ weights, index=control_mat.index
    )
...
print("top_weights=")
print(
    pd.Series(weights, index=controls)
...
for unit in controls:
    ...
    placebo_abs.append(
        abs(float(gap_c.loc[gap_c.index >= TREAT_START].mean()))
    )
...
axes[0].plot(
    treated.index, treated.values, label="Treated market", linewidth=2
)
axes[0].plot(
    synth.index, synth.values, label="Synthetic control", linewidth=2
)
axes[0].axvline(TREAT_START, color="0.2", linestyle="--", linewidth=1)
axes[0].set_ylabel("Outcome")
axes[0].legend()
axes[0].set_title(
    "Synthetic control: treated market versus donor-weighted peers"
)
...
fig.savefig(
    img_dir / "ch9_synthetic_control.png", dpi=300, bbox_inches="tight"
)
plt.close()
