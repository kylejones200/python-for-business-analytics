...
def make_cox_units(n_units=80, seed=7):
    ...
    for unit in range(1, n_units + 1):
        ...
        life = int(np.clip(220 - 2.2 * (sensor - 50.0) + rng.normal(0, 18), 60, 260))
        censored = unit % 5 == 0
        duration = life if not censored else max(30, int(life * 0.70))
        rows.append({"duration": duration, "event": 0 if censored else 1, "sensor_11": sensor})
    return pd.DataFrame(rows)

df = make_cox_units()
res = PHReg(df["duration"], df[["sensor_11"]], status=df["event"]).fit(disp=False)
beta = float(res.params[0])
hr = float(np.exp(beta))
print(res.summary())
print("sensor_11 coefficient: {:.3f}".format(beta))
print("Hazard ratio exp(beta): {:.3f}".format(hr))
print("A 1-unit increase in sensor_11 multiplies the hazard by {:.3f}.".format(hr))

df["high_sensor"] = (df["sensor_11"] >= df["sensor_11"].median()).astype(int)
fig, ax = plt.subplots(figsize=(8, 5))
for label, mask in (("Low sensor_11", df["high_sensor"] == 0), ("High sensor_11", df["high_sensor"] == 1)):
    sub = df.loc[mask]
...
fig.savefig(img_dir / "ch7_cox_partial_effects.png", dpi=300, bbox_inches="tight")
plt.close(fig)
