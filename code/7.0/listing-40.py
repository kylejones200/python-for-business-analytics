...
def make_weibull_units(n_units=80, seed=7):
    ...
    for unit in range(1, n_units + 1):
        life = float(
            stats.weibull_min(c=1.6, scale=160).rvs(random_state=rng)
        )
...
ax.plot(
    t_grid,
    1.0 - stats.weibull_min(c=w_shape, loc=w_loc, scale=w_scale).cdf(t_grid),
    label="Weibull",
)
ax.plot(
    t_grid,
    1.0 - stats.lognorm(s=ln_shape, loc=ln_loc, scale=ln_scale).cdf(t_grid),
    label="Lognormal",
)
...
fig.savefig(
    img_dir / "ch7_survival_models_comparison.png",
    dpi=300,
    bbox_inches="tight",
)
plt.close(fig)
