...
for kind in ("spherical", "exponential"):
    pred = predict(kind, x[train], y[train], z[train], x[~train], y[~train], 4.0, 28.0, 12_000.0)
    obs = z[~train]
...
axes[0].bar(results.keys(), [r["rmse"] for r in results.values()], color="0.55", edgecolor="black")
axes[0].set_ylabel("RMSE")
axes[0].set_title("Holdout RMSE")
axes[1].bar(results.keys(), [r["r2"] for r in results.values()], color="0.55", edgecolor="black")
...
fig.savefig(img_dir / "ch8_variogram_model_comparison.png", dpi=300, bbox_inches="tight")
plt.close(fig)
