...
for i, yy in enumerate(gy):
    for j, xx in enumerate(gx):
        surface[i, j] = ordinary_krige(x, y, demand, xx, yy, nugget, psill, vrange)

imax = np.unravel_index(np.argmax(surface), surface.shape)
best_x, best_y = gx[imax[1]], gy[imax[0]]

fig, ax = plt.subplots(figsize=(7, 6))
im = ax.imshow(surface, origin="lower", extent=[0, 50, 0, 50], cmap="RdYlGn", alpha=0.85)
ax.scatter(x / 1000.0, y / 1000.0, s=12, c="0.15", alpha=0.5, label="Customers")
ax.scatter([best_x / 1000.0], [best_y / 1000.0], marker="*", s=220, c="red", label="Highest predicted demand")
...
fig.savefig(img_dir / "store_location_optimization.png", dpi=300, bbox_inches="tight")
plt.close(fig)
print("Saved img/store_location_optimization.png")
print("Recommended site: easting={:.1f} km, northing={:.1f} km".format(best_x / 1000.0, best_y / 1000.0))
