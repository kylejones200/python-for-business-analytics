"""Predict property values with a spatial block holdout."""
...
pred = ordinary_krige_points(
    x[train], y[train], value[train], x[test], y[test], 20.0, 180.0, 16_000.0
)
resid = value[test] - pred
rmse = float(np.sqrt(np.mean(resid**2)))
mae = float(np.mean(np.abs(resid)))
r2 = 1.0 - float(
    np.sum(resid**2) / np.sum((value[test] - value[test].mean()) ** 2)
)
...
lo, hi = float(min(value[test].min(), pred.min())), float(
    max(value[test].max(), pred.max())
)
...
fig.savefig(
    img_dir / "property_value_prediction.png", dpi=300, bbox_inches="tight"
)
plt.close(fig)
