"""Analyze price variations using pivot tables.

This module demonstrates using pandas pivot tables to analyze relationships
between categorical features (amenities) and continuous variables (price).
Readers learn how to create cross-tabulations and multi-dimensional analyses.

Chapter: Introduction to Natural Language Processing for Business Analytics
Source: nlp_section.tex
Extracted listing: 24
"""
...
def main() -> None:
    """Analyze price variations by amenities using pivot tables."""
    ...
    # How does price vary by pet-friendliness?
    logger.info("\nAverage Price by Pet Policy:")
    price_by_pet = pd.pivot_table(
        df_listings, values="price", columns="pet", aggfunc="mean"
    )
    logger.info(f"\n{price_by_pet}")

    # Two-way analysis: pets AND cable TV
    logger.info("\nAverage Price by Pet Policy and Cable TV:")
