"""Fit and plot the revenue-per-employee regression used as the worked example."""
...
print(f"employees range: {int(df['employees'].min())} to {int(df['employees'].max())}")
print(f"prediction at 250 employees: {b0 + b1 * 250:,.0f}")

fig, ax = plt.subplots(figsize=(8, 5))
ax.scatter(df["employees"], df["annual_revenue_usd"] / 1e6, s=8, alpha=0.35, color="0.35")
...
fig.savefig(img_dir / "ch5_revenue_employees.png", dpi=300, bbox_inches="tight")
plt.close(fig)
