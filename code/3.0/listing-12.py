...
This script demonstrates calculating and visualizing moving averages on time
series data. Readers learn to smooth time series data and identify trends
...
def main():
    ...
    ax.plot(
        df["Date"], df["MA_7"], label="7-Day MA", linewidth=2, color="#5E81AC"
    )
    ax.plot(
        df["Date"],
        df["MA_14"],
    ...
    ax.set_title(
        "Time Series with Multiple Moving Averages", fontsize=13, pad=15
    )
    ...
    plt.savefig(
        img_dir / "ch3_moving_averages.png", dpi=150, bbox_inches="tight"
    )
