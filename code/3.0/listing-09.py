...
This script demonstrates creating multi-panel dashboards with multiple
visualizations. Readers learn to combine multiple plots into a cohesive
dashboard layout.
"""
...
def main():
    ...
    revenue = np.array(
        [120, 125, 135, 140, 155, 160, 158, 165, 170, 180, 195, 210]
    )
    ...
    ax1.bar(
        x + width / 2, costs, width, color=colors[1], alpha=0.7, label="Costs"
    )
    ...
    ax2.text(
        ...
        bbox=dict(
            boxstyle="round", facecolor="#A3BE8C", alpha=0.3, edgecolor="none"
        ),
    )
    ...
    plt.savefig(
        img_dir / "ch3_business_dashboard.png", dpi=150, bbox_inches="tight"
    )
    ...
    logger.info(
        f"  Total Revenue: ${revenue.sum()}K "
        f"({((revenue[-1]-revenue[0])/revenue[0]*100):+.1f}%)"
    )
    logger.info(
        f"  Total Profit:  ${profit.sum()}K "
        f"({((profit[-1]-profit[0])/profit[0]*100):+.1f}%)"
    )
    logger.info(
        f"  Customers:     {customers[-1]:,} "
        f"({((customers[-1]-customers[0])/customers[0]*100):+.1f}%)"
    )
    logger.info(
        f"  Satisfaction:  {satisfaction[-1]:.1f}/10 "
        f"({satisfaction[-1]-satisfaction[0]:+.1f} pts)"
    )
