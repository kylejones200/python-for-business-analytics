...
Readers learn to decompose time series into trend, seasonal, and residual components.
"""
import logging
import os
...
def main():
    ...
    axes[0].set_title("Seasonal Decomposition of Time Series", fontsize=13, pad=10)
    ...
    # Seasonal
    axes[2].plot(df.index, decomposed.seasonal, color="#A3BE8C", linewidth=1.5)
    remove_chartjunk(axes[2])
    axes[2].set_ylabel("Seasonal", fontsize=10)

    # Residual
    axes[3].plot(df.index, decomposed.resid, color="#BF616A", linewidth=0.8, alpha=0.7)
    axes[3].axhline(0, color="#4C566A", linestyle="--", linewidth=1, alpha=0.5)
    ...
    plt.savefig(img_dir / "ch3_seasonal_decomposition.png", dpi=150, bbox_inches="tight")
    logger.info(f"Saved figure to {img_dir / 'ch3_seasonal_decomposition.png'}")

    # Log component statistics
    logger.info("\nComponent Statistics:")
    logger.info(f"Trend range: {decomposed.trend.min():.2f} to {decomposed.trend.max():.2f}")
