...
This script demonstrates creating bar charts with value labels and conditional
coloring. Readers learn to visualize categorical data and highlight important
categories.
"""
...
def main():
    ...
    plt.savefig(
        img_dir / "ch3_sales_by_category.png", dpi=150, bbox_inches="tight"
    )
    ...
    logger.info(f"\nTotal Sales: ${total}K")
    logger.info(
        f"\nTop Category: {categories[sales.index(max(sales))]} "
        f"(${max(sales)}K, {max(sales)/total*100:.1f}%)"
    )
    logger.info(
        f"Bottom Category: {categories[sales.index(min(sales))]} "
        f"(${min(sales)}K, {min(sales)/total*100:.1f}%)"
    )
